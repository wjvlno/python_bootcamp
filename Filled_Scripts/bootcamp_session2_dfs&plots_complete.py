...
[x for x in dir(df) if "to_" in x] # list built-in methods that contain "to_"

[x for x in dir(df) if x.startswith("to_")] # methods that start with "to_"
################################################################################
...
df[df["Make"].isin(["Ferrari"])]
df[df['Make'] == "Ferrari"] # same result
...
df[(df['Make'] == "Ferrari") & (df['Model'] == "California")] # Make is Ferrari AND ('&') model is California
################################################################################
## Setting new values
...
# Add: Median MPG
df['MPG_median'] = df["MPG"].median()

# Add: interquartile range for MPG
df['MPG_IQR'] = df["MPG"].quantile(.75) - df["MPG"].quantile(.25)
...
df = df.assign(MPG_vs_makeMean = np.select([df.MPG < df.MPG_mean_byMake, df.MPG > df.MPG_mean_byMake], ['Below', "Above"], np.nan))
...
df.loc[most_expensive_indices, "Most_Expensive_inMake"] = True # Task
...
df['Price'] = df['Price'].apply(clean_price) # TASK: fill in right side
...
plt.figure(figsize=(12, 6))
sns.violinplot(x='Make', y='Price', data=df[~(df['Make'] == 'Ferrari')], inner=None, color=".8")
sns.boxplot(x='Make', y='Price', data=df[~(df['Make'] == 'Ferrari')], whis=[0, 100], width=.2, palette="vlag")
plt.title('Boxplot with Violins for Price within Make (Without Ferrari Models)')
plt.show()
################################################################################
